...
export function signalAlert(signal: Signal, now: number) {
  ...
  return {
    ...
    title: `${signal.side} ${signal.symbol} · ${confirmed ? "dikonfirmasi AI" : "kandidat teknikal"}`,
    body: `Entry ${signal.plan.entry} · Stop ${signal.plan.stop} · Target ${signal.plan.target}. ${confirmed ? "Buka detail untuk meninjau simulasi." : "Belum dikonfirmasi AI; periksa detail sebelum latihan."}`,
    confirmed,
  };
}
